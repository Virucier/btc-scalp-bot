...
def send_telegram_message(message: str):
    if not TELEGRAM_TOKEN or not CHAT_ID:
        print("⚠️ TELEGRAM_TOKEN ou CHAT_ID manquant dans .env")
        print(message)
        return
...
def run_bot():
    ...
    print(f"Capital : {CAPITAL}$ | Vérification toutes les {CHECK_INTERVAL_MINUTES} min\n")
    ...
    while True:
        try:
            ...
            if df_15m is None or df_1h is None:
                print("Erreur données. Nouvelle tentative dans 2 min...")
                time.sleep(120)
                continue
            ...
            if signal:
                ...
                if last_signal_time is None or (current_time - last_signal_time).total_seconds() > 1800:
                    ...
                    print("Signal détecté mais cooldown actif.")
            else:
                print(f"[{datetime.now().strftime('%H:%M')}] Aucun setup valide.")
